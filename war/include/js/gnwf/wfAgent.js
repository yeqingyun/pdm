...
function f_onCheckRow(checked, data){
	if (checked)
		addCheckedCustomer(data.userId);
	else
		removeCheckedCustomer(data.userId);
}
...
function add() {
	$.ligerDialog.open({title:'新增对话框：WfAgent', height: 560, width: 780,url: './WfAgent!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：WfAgent', height: 560, width: 780,url: './WfAgent!edit.shtml?wfAgent.userId='+data.userId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：WfAgent', height: 560, width: 780,url: './WfAgent!view.shtml?wfAgent.userId='+data.userId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'wfAgent.userId', value: $("#userId").val()},
			{name: 'wfAgent.agentId', value: $("#agentId").val()},
			{name: 'wfAgent.flowId', value: $("#flowId").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"wfAgent.userId":"'+$("#userId").val()+'",';
				str += '"wfAgent.agentId":"'+$("#agentId").val()+'",';
				str += '"wfAgent.flowId":"'+$("#flowId").val()+'"';
				str += '}';

				$.post("WfAgent!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfAgent!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './WfAgent!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：WfAgent', height: 560, width: 780,url: './WfAgent!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfAgent!dow.shtml");
				$("#form1").submit();
			}
		}
	);
