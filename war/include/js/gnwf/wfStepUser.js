...
function add() {
	$.ligerDialog.open({title:'新增对话框：WfStepUser', height: 560, width: 780,url: './WfStepUser!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：WfStepUser', height: 560, width: 780,url: './WfStepUser!edit.shtml?wfStepUser.stepId='+data.stepId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：WfStepUser', height: 560, width: 780,url: './WfStepUser!view.shtml?wfStepUser.stepId='+data.stepId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'wfStepUser.stepId', value: $("#stepId").val()},
			{name: 'wfStepUser.userId', value: $("#userId").val()},
			{name: 'wfStepUser.userType', value: $("#userType").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"wfStepUser.stepId":"'+$("#stepId").val()+'",';
				str += '"wfStepUser.userId":"'+$("#userId").val()+'",';
				str += '"wfStepUser.userType":"'+$("#userType").val()+'"';
				str += '}';

				$.post("WfStepUser!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfStepUser!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './WfStepUser!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：WfStepUser', height: 560, width: 780,url: './WfStepUser!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfStepUser!dow.shtml");
				$("#form1").submit();
			}
		}
	);
