...
function retnPop() {
	window.parent.addto(getOptz("to"));
	window.parent.addcc(getOptz("cc"));
	closePop();
}
...
function setOptz(id,_to) {
	if(_to != "") {
		var _tos = _to.split(",");
		for(i=0; i<_tos.length; i++) {
			var _optz = new Option(_tos[i],_tos[i]);
...
//				document.getElementById(to).options[i].value = getXmldocNode(xmldom,"METADATA/ITEM["+i+"]/mailAddr");
//				document.getElementById(to).options[i].text = getXmldocNode(xmldom,"METADATA/ITEM["+i+"]/userName")+"<"+getXmldocNode(xmldom,"METADATA/ITEM["+i+"]/mailAddr")+">";
//			}
//		}
//	}
//	catch(e) {
