...
function add() {
	$.ligerDialog.warn('请在人力资源部分模块，增加工作岗位。');
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：WfJob', height: 560, width: 780,url: './WfJob!edit.shtml?wfJob.jobId='+data.jobId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：WfJob', height: 560, width: 780,url: './WfJob!view.shtml?wfJob.jobId='+data.jobId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'hrJobCfg.jobId', value: $("#jobId").val()},
			{name: 'hrJobCfg.comId', value: $("#comId").val()},
			{name: 'hrJobCfg.deptId', value: $("#deptId").val()},
			{name: 'hrJobCfg.status', value: $("#status").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var jobId = $("#jobId").val();
				var arrFlow = $("input[name='_flows']");
				var str = '{';
				str += '"wfJob.jobId":"'+jobId+'"';
			    for (var i=0;i<arrFlow.length;i++) {
			         if(arrFlow[i].checked == true) {
			         	str += ',"wfJobs['+i+'].jobId":"'+jobId+'"';
			        	str += ',"wfJobs['+i+'].flowId":"'+arrFlow[i].value+'"';
			        	//流程子任务选择处理
			        	var arrFile = $("input[name='file"+i+"']");
			        	for(var j = 0; j < arrFile.length ; j ++) {
			        		if(arrFile[j].checked == true) {
				        		str += ',"wfJobs['+i+'].isUpdOrLoad":"'+(j+1)+'"';
			        		}
			        	}
			        	var arrAnnex = $("input[name='annex"+i+"']");
			        	for(var j = 0; j < arrAnnex.length ; j ++) {
			        		if(arrAnnex[j].checked == true) {
				        		str += ',"wfJobs['+i+'].annexUpdOrLoad":"'+(j+1)+'"';
			        		}
			        	}
			         }
			    }
			    var arrUsr = $("input[name='_usrs']");
			     for (var i=0;i<arrUsr.length;i++) {
			         if(arrUsr[i].checked == true) {
			         	str += ',"wfJobUsers['+i+'].jobId":"'+jobId+'"';
			        	str += ',"wfJobUsers['+i+'].userId":"'+arrUsr[i].value+'"';
			         }
			    }
				str += '}';
				$.post("WfJob!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfJob!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './WfJob!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：WfJob', height: 560, width: 780,url: './WfJob!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfJob!dow.shtml");
				$("#form1").submit();
			}
		}
	);
