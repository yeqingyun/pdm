...
function add() {
	$.ligerDialog.open({title:'新增对话框：WfDept', height: 560, width: 780,url: './WfDept!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：WfDept', height: 560, width: 780,url: './WfDept!edit.shtml?wfDept.flowId='+data.flowId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：WfDept', height: 560, width: 780,url: './WfDept!view.shtml?wfDept.flowId='+data.flowId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'wfDept.flowId', value: $("#flowId").val()},
			{name: 'wfDept.deptId', value: $("#deptId").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"wfDept.flowId":"'+$("#flowId").val()+'",';
				str += '"wfDept.deptId":"'+$("#deptId").val()+'"';
				str += '}';

				$.post("WfDept!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfDept!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './WfDept!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：WfDept', height: 560, width: 780,url: './WfDept!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","WfDept!dow.shtml");
				$("#form1").submit();
			}
		}
	);
