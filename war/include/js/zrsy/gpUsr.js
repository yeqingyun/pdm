...
function add() {
	$.ligerDialog.open({title:'新增对话框：GpUsr', height: 560, width: 780,url: './GpUsr!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：GpUsr', height: 560, width: 780,url: './GpUsr!edit.shtml?gpUsr.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：GpUsr', height: 560, width: 780,url: './GpUsr!view.shtml?gpUsr.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'gpUsr.gpId', value: $("#gpId").val()},
			{name: 'gpUsr.usrId', value: $("#usrId").val()}
...
function sav() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"gpUsr.gpId":"'+$("#gpId").val()+'",';
				str += '"gpUsr.usrId":"'+$("#usrId").val()+'"';
				str += '}';

				$.post("GpUsr!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpUsr!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './GpUsr!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：GpUsr', height: 560, width: 780,url: './GpUsr!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpUsr!dow.shtml");
				$("#form1").submit();
			}
		}
	);
