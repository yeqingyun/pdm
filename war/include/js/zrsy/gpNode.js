...
function add() {
	$.ligerDialog.open({title:'新增对话框：GpNode', height: 560, width: 780,url: './GpNode!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：GpNode', height: 560, width: 780,url: './GpNode!edit.shtml?gpNode.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：GpNode', height: 560, width: 780,url: './GpNode!view.shtml?gpNode.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'gpNode.gpId', value: $("#gpId").val()},
			{name: 'gpNode.nodeId', value: $("#nodeId").val()}
...
function sav() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"gpNode.gpId":"'+$("#gpId").val()+'",';
				str += '"gpNode.nodeId":"'+$("#nodeId").val()+'"';
				str += '}';

				$.post("GpNode!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpNode!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './GpNode!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：GpNode', height: 560, width: 780,url: './GpNode!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpNode!dow.shtml");
				$("#form1").submit();
			}
		}
	);
