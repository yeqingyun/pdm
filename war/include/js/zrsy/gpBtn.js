...
function add() {
	$.ligerDialog.open({title:'新增对话框：GpBtn', height: 560, width: 780,url: './GpBtn!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：GpBtn', height: 560, width: 780,url: './GpBtn!edit.shtml?gpBtn.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：GpBtn', height: 560, width: 780,url: './GpBtn!view.shtml?gpBtn.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'gpBtn.gpId', value: $("#gpId").val()},
			{name: 'gpBtn.nodeId', value: $("#nodeId").val()},
			{name: 'gpBtn.btnId', value: $("#btnId").val()}
...
function sav() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"gpBtn.gpId":"'+$("#gpId").val()+'",';
				str += '"gpBtn.nodeId":"'+$("#nodeId").val()+'",';
				str += '"gpBtn.btnId":"'+$("#btnId").val()+'"';
				str += '}';

				$.post("GpBtn!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpBtn!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './GpBtn!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：GpBtn', height: 560, width: 780,url: './GpBtn!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpBtn!dow.shtml");
				$("#form1").submit();
			}
		}
	);
