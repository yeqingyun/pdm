...
function add() {
	$.ligerDialog.open({title:'新增对话框：GpSy', height: 560, width: 780,url: './GpSy!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：GpSy', height: 560, width: 780,url: './GpSy!edit.shtml?gpSy.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：GpSy', height: 560, width: 780,url: './GpSy!view.shtml?gpSy.gpId='+data.gpId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'gpSy.gpId', value: $("#gpId").val()},
			{name: 'gpSy.syId', value: $("#syId").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"gpSy.gpId":"'+$("#gpId").val()+'",';
				str += '"gpSy.syId":"'+$("#syId").val()+'"';
				str += '}';
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpSy!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './GpSy!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：GpSy', height: 560, width: 780,url: './GpSy!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","GpSy!dow.shtml");
				$("#form1").submit();
			}
		}
	);
