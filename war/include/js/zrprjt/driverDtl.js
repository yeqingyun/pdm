...
function add() {
	$.ligerDialog.open({title:'新增对话框：DriverDtl', height: 560, width: 780,url: './DriverDtl!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：DriverDtl', height: 560, width: 780,url: './DriverDtl!edit.shtml?driverDtl.driveId='+data.driveId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：DriverDtl', height: 560, width: 780,url: './DriverDtl!view.shtml?driverDtl.driveId='+data.driveId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'driverDtl.driveId', value: $("#driveId").val()},
			{name: 'driverDtl.flowId', value: $("#flowId").val()},
			{name: 'driverDtl.taskId', value: $("#taskId").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"driverDtl.driveId":"'+$("#driveId").val()+'",';
				str += '"driverDtl.flowId":"'+$("#flowId").val()+'",';
				str += '"driverDtl.taskId":"'+$("#taskId").val()+'"';
				str += '}';

				$.post("DriverDtl!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","DriverDtl!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './DriverDtl!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：DriverDtl', height: 560, width: 780,url: './DriverDtl!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","DriverDtl!dow.shtml");
				$("#form1").submit();
			}
		}
	);
