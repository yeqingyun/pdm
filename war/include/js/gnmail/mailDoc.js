...
function add() {
	$.ligerDialog.open({title:'新增对话框：MailDoc', height: 560, width: 780,url: './MailDoc!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：MailDoc', height: 560, width: 780,url: './MailDoc!edit.shtml?mailDoc.docId='+data.docId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：MailDoc', height: 560, width: 780,url: './MailDoc!view.shtml?mailDoc.docId='+data.docId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'mailDoc.docId', value: $("#docId").val()},
			{name: 'mailDoc.mailId', value: $("#mailId").val()},
			{name: 'mailDoc.docName', value: $("#docName").val()},
			{name: 'mailDoc.uriLink', value: $("#uriLink").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"mailDoc.docId":"'+$("#docId").val()+'",';
				str += '"mailDoc.mailId":"'+$("#mailId").val()+'",';
				str += '"mailDoc.docName":"'+$("#docName").val()+'",';
				str += '"mailDoc.uriLink":"'+$("#uriLink").val()+'"';
				str += '}';

				$.post("MailDoc!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","MailDoc!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './MailDoc!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：MailDoc', height: 560, width: 780,url: './MailDoc!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","MailDoc!dow.shtml");
				$("#form1").submit();
			}
		}
	);
