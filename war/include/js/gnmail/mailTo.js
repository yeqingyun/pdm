...
function add() {
	$.ligerDialog.open({title:'新增对话框：MailTo', height: 560, width: 780,url: './MailTo!add.shtml',showMax: true, showToggle: true, showMin: true, isResize: true});
}
function edi() {
	...
	$.ligerDialog.open({title:'编辑对话框：MailTo', height: 560, width: 780,url: './MailTo!edit.shtml?mailTo.mailId='+data.mailId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function vie(data) {
	$.ligerDialog.open({title:'查看对话框：MailTo', height: 560, width: 780,url: './MailTo!view.shtml?mailTo.mailId='+data.mailId,showMax: true, showToggle: true, showMin: true, isResize: true});
}
function sea() {
	gridManager.setOptions({
		parms: [
			{name: 'mailTo.mailId', value: $("#mailId").val()},
			{name: 'mailTo.toId', value: $("#toId").val()},
			{name: 'mailTo.toMail', value: $("#toMail").val()},
			{name: 'mailTo.toType', value: $("#toType").val()},
			{name: 'mailTo.toName', value: $("#toName").val()}
...
function saver() {
	$.ligerDialog.confirm('确定要保存记录？', 
		function (type) { 
			if(type) {
				var str = '{';
				str += '"mailTo.mailId":"'+$("#mailId").val()+'",';
				str += '"mailTo.toId":"'+$("#toId").val()+'",';
				str += '"mailTo.toMail":"'+$("#toMail").val()+'",';
				str += '"mailTo.toType":"'+$("#toType").val()+'",';
				str += '"mailTo.toName":"'+$("#toName").val()+'"';
				str += '}';

				$.post("MailTo!sav.shtml",
					JSON.parse(str),
					function(data) {
...
function exp() {
	$.ligerDialog.confirm('确定要导出记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","MailTo!exp.shtml");
				$("#form1").submit();
			}
		}
	);
...
function imp() {
	$.ligerDialog.confirm('确定要导入记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'对话框：文件导入', height: 160, width: 500,url: './MailTo!imp.shtml'});
			}
		}
	);
}
function prn() {
	...
	$.ligerDialog.confirm('确定要打印记录？', 
		function (type) { 
			if(type) {
				$.ligerDialog.open({title:'打印对话框：MailTo', height: 560, width: 780,url: './MailTo!prn.shtml?choices='+checkedCustomer.join(',')});
			}
		}
	);
}
function dow() {
	...
	$.ligerDialog.confirm('确定要下载记录？', 
		function (type) { 
			if(type) {
				$("#form1").attr("action","MailTo!dow.shtml");
				$("#form1").submit();
			}
		}
	);
