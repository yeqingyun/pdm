...
<form id ="prjtForm" name ="prjtForm" action="PrjtDef!sav.shtml"  method="post" enctype="multipart/form-data">
	...
	<table width="90%">
	   ...
	   <tr>
			...
			<td>
			 ...
			 <select id="devDeptNameID" name="prjtDef.devDeptNameID" style="width: 196px">
					<option value = "1" <c:if test="${prjtDef.devDeptNameID==1}">selected</c:if>>研发一部</option>
					<option value = "2" <c:if test="${prjtDef.devDeptNameID==2}">selected</c:if>>研发二部</option>
					<option value = "3" <c:if test="${prjtDef.devDeptNameID==3}">selected</c:if>>研发三部</option>
	   ...
	   <!-- 
		...
		<tr>
			<td height="24" width="90" >状态：</td><td>
			   <select id="status" name="prjtDef.status" style="width: 196px">
					<option value = "0" <c:if test="${prjtDef.status==0}">selected</c:if>>无效</option>
					<option value = "1" <c:if test="${prjtDef.status==1}">selected</c:if>>新创建</option>
					<option value = "2" <c:if test="${prjtDef.status==2}">selected</c:if>>已启动</option>
					<option value = "3" <c:if test="${prjtDef.status==3}">selected</c:if>>暂停</option>
					<option value = "4" <c:if test="${prjtDef.status==4}">selected</c:if>>已结束</option>
				</select>
			</td>
		</tr>
