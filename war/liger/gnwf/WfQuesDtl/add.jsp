...
<%--
	<table width="90%">
		<tr>
			...
			<td>
				<select id="status" name="wfQuesDtl.status" style="width: 200px;"<c:if test="${wfQuesDtl.status<300}">disabled</c:if>>
					<option value="100"<c:if test="${wfQuesDtl.status==100}">selected</c:if> disabled="disabled">发现</option>
					<option value="200"<c:if test="${wfQuesDtl.status==200}">selected</c:if> disabled="disabled">回复</option>
...
<form>
	...
	<table width="90%">
		...
		<tr>
			...
			<td align=left><input type="text" id="userId" name="wfQuesDtl.userId" /></td>
		</tr>
		<tr>
