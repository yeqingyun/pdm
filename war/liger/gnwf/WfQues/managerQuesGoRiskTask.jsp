...
$(function () {
    ...
    function labelRiskStatus(quesRespId) {
    	//alert(quesRespId);
    	$.ligerDialog.confirm("你确认走转风险评估流程吗?", function(data){
    		if(data){
    			var str = '{';
    			str += '"wfRisk.riskConsequence":"' + $("#wfRisk_riskConsequence").val() + '",';
    			str += '"wfRisk.preventiveMeasures":"' + $("#wfRisk_preventiveMeasures").val() + '",';
    			//alert($("#wfRisk_impTime").val())
    			str += '"wfRisk.impTime":"' + $("#wfRisk_impTime").val() + '",';
    			str += '"wfQues.quesId":"' + $("#wfQues_quesId").val() + '",';
    			
    			//要改过来
    	   		str += '"deptManagerID":"' + $("#userId1").val() + '",';
    			...
    			$.post("WfQues!labelRiskStatus.shtml",
    					...
    					function(data) {
    							var arr = data.split(",");
   							 var WfNo1234=	arr[1].split(":")[1];
   							var TaskId1234=	arr[2].split(":")[1];
   							var StepId1234=	arr[3].split(":")[1];
    						 $.ligerDialog.success(arr[0].split(":")[1],"提示",function(){
    							...
    							window.location = "./WfRdView!wfTaskView.shtml?wfRd.wfNo="+WfNo1234+"&currentTaskId="+TaskId1234+"&taskStepId="+StepId1234;
    						 });
    					},
    					"text"
    		    );
        ...
        <c:forEach items="${wfQues.quesRespList}" var="re">
         <div class="area01" id = "风险说明及风险后果">
            ...
            <div class="area01Con">
                ...
                <table class="table01 table02">
                    <tbody>
                    	<tr>
                            <th nowrap="nowrap">风险说明及风险后果</th>
                            <td nowrap="nowrap">
							            <textarea class="tableTextarea01" id= "wfRisk_riskConsequence"  name="wfRisk_riskConsequence" autofocus="autofocus" 
							            value="<c:out value="${wfRisk.riskConsequence}"/>"> </textarea>
                            </td>
                        </tr>
                        <tr>
                            <th nowrap="nowrap">拟采取的预防措施</th>
                            <td nowrap="nowrap">
                                    <textarea class="tableTextarea01" id= "wfRisk_preventiveMeasures"  name="wfRisk_preventiveMeasures" autofocus="autofocus" 
							            value="<c:out value="${wfRisk.preventiveMeasures}"/>"> </textarea>
                            </td>
                        </tr>
                        
                        <tr>
                            <th height="24" width="90" align="center" >拟导入时间</th>
                            <td >
									<input type="text" id="wfRisk_impTime" name="wfRisk_impTime" size="50" autofocus="autofocus"   value="<c:out value="${wfRisk.impTime}"/>"/>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2" class="padd0">
                                <table>
                                    <tbody>
                                        <tr>
                                            <th nowrap="nowrap">指定接收部门经理</th>
											<td >
											 <input type="hidden" id="userId1" value="<c:out value="${step.idText}"/>"/> 
													 <input type="text"  
														id="userName1"
														size="38"
														value="<c:out value="${step.idText}"/>">
														<input type="button" value="选择成员" onclick="selectNextStepUsers(this,1);"></input>
         ...
         <tr>
                            <td class="noBorR"></td>
                                          ...
                                          &nbsp;&nbsp;&nbsp;&nbsp;  <button id="showGoRiskBtn" class="tableBut"  onclick="labelRiskStatus(<c:out value="${re.id}"/>);"  >提交并转风险评估</button>
                                           </td>
       	</tr>
       	</div>
       	 </c:forEach>
