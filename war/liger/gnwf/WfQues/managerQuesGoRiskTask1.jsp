...
<div class="listTable clearfix" style="margin:10px 20px;" >
		<table cellpadding=0 cellspacing=0.5 width=700px
			...
			<tr>
				...
				<td colspan="3"  align=center><c:out value="${wfQues.prjtNm}"/></td>

			</tr>
			
			<tr>
				<th nowrap="nowrap">阶段</th>
			...
			<tr>
				<th>问题描述</th>
				<td colspan="3"><textarea style="width: 600px; height: 50px;"
						class="tableTextarea01"><c:out
							value="${wfQues.quesText}" escapeXml="false" /></textarea></td>
				<!-- <th>问题描述：<font color="red">&nbsp;*</font></th>
			<td colspan="3">
				<textarea id="quesText" name="wfQues.quesText"  validate="{required:true}"></textarea>  
			</td> -->
			</tr>

			<tr>
				<th nowrap="nowrap">责任人</th>
				<td nowrap="nowrap" id="responsible"><c:out
						value="${wfQues.usrName}" /></td>


				<th nowrap="nowrap">问题提出者</th>
				<td nowrap="nowrap"><span class="padLR5"><c:out
							value="${wfQues.createUsr}" /></span></td>
			...
			<tr>
				...
				<td colspan="3"><img align="middle"
					src="./include/img/workflow/<c:out value="${wfQues.fileIcon}"/>" />
					<c:choose>
						<c:when test="${fn:contains(wfQues.fileNo,'/')}">
							<a class="colorBlue padLR5"
								href='javascript:downloadOrViewFile("<c:out value="${wfQues.quesId}"/>","<c:out value="${wfQues.fileName}"/>");'>
						</c:when>
						<c:otherwise>
							<a class="colorBlue padLR5"
								href='<c:out value="${initParam.gngile_download_URL}"/>?fileNo=<c:out value="${wfQues.fileNo}"/>'>
						</c:otherwise>
					</c:choose> <c:out value="${wfQues.fileName}" /> </a></td>
			</tr>
