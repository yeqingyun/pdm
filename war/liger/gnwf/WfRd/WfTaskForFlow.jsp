...
function selectPrjtUsers(){
	...
	function rejectJob(){		//退回任务
		if(document.getElementById("currentTask.createBy").value == -1){
			$.ligerDialog.warn('系统所发任务不可退回!');
		}else{
			var taskId = document.getElementById("currentTask.taskId").value;
			$.ligerDialog.prompt('您确定要退回任务吗？','请写退回原因(字数不得超过50个字)，谢谢',true, function (yes,str) {
				if(yes) {
...
<div>
 ...
 <form id="wfRd-form" name="wfRd-form"   method="post" action="WfRdView.shtml" onsubmit="return saveJobCheck();" enctype="multipart/form-data">
 	<input type="hidden" id="isForFlow" name="isForFlow" value="1"/>
...
var OnMessage = function (e) {
	switch(e.data){
		...
		case 'rejectJob': rejectJob();break;
		case 'saveJob': saveJob();break;
		case 'setAgentBy': setAgentBy();break;
		case 'stopWfinfo': stopWfinfo();break;
	}
}
//初始化iframe中的监听器
function init() {
