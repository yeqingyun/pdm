...
<c:choose>
    <c:when test="${E1.isEdit == '1'}">
        <input type="radio" name="fieldContents[<c:out value="${E1.count}"/>].fieldText" value="1"
            <c:if test="${E1.fieldText == 1 || empty E1.fieldText}">checked</c:if> />SMT(PCBA)&nbsp;
        <input type="radio" name="fieldContents[<c:out value="${E1.count}"/>].fieldText" value="2"
            <c:if test="${E1.fieldText == 2 || empty E1.fieldText}">checked</c:if> />整机&nbsp;
    </c:when>
    <c:otherwise>
        <input type="radio" disabled="disabled" <c:if test="${E1.fieldText == 1}">checked</c:if> />SMT(PCBA)&nbsp;
        <input type="radio" disabled="disabled" <c:if test="${E1.fieldText == 2}">checked</c:if> />整机&nbsp;
    </c:otherwise>
</c:choose>
    </td>
    <td align="right">
...
<table width="680" border=1 bordercolor=#000000 align=center cellpadding=0 cellspacing=0  style='border-collapse:collapse;font-size:11.0pt'>
  <tr height="30">
    ...
    <td width="180"><div align="center">要求试产日期</div></td>
    <td width="230">&nbsp;
	<c:choose>
  ...
  <tr>
    <td height="450" colspan="4" valign="top">试产方案及注意事项：
		<br/>
	&nbsp;
    ...
    <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<c:choose>
			    ...
			    <c:otherwise>
			        <textarea rows="4" cols="70" style="width: 670px"  
					<c:choose>
					    <c:when test="${E15.isEdit == '1'}">
					    </c:when>
					    <c:otherwise>
					 ...
					 class="textarea-hidden"><c:out value="${fieldContents[14].fieldText}"/></textarea>
			    </c:otherwise>
			</c:choose>
			</td>
		</tr>
...
<c:choose>
    <c:when test="${E24.isEdit == '1'}">
        ...
        <input name="fieldContents[<c:out value="${E25.count}"/>].fieldText"  type="checkbox" value="2"
            <c:if test="${E25.fieldText == '2'}">checked</c:if> />计调部&nbsp;
        <input name="fieldContents[<c:out value="${E26.count}"/>].fieldText"  type="checkbox" value="3"
            <c:if test="${E26.fieldText == '3'}">checked</c:if> />生产部&nbsp;
        <input name="fieldContents[<c:out value="${E27.count}"/>].fieldText"  type="checkbox" value="4"
            <c:if test="${E27.fieldText == '4'}">checked</c:if> />品质部&nbsp;
        <input name="fieldContents[<c:out value="${E28.count}"/>].fieldText"  type="checkbox" value="5"
            <c:if test="${E28.fieldText == '5'}">checked</c:if> />工程部 &nbsp;
    </c:when>
    <c:otherwise>
        <input type="checkbox" disabled="disabled" <c:if test="${fieldContents[23].fieldText == '1'}">checked</c:if> />采购部&nbsp;
        <input type="checkbox" disabled="disabled" <c:if test="${fieldContents[24].fieldText == '2'}">checked</c:if> />计调部&nbsp;
        <input type="checkbox" disabled="disabled" <c:if test="${fieldContents[25].fieldText == '3'}">checked</c:if> />生产部&nbsp;
        <input type="checkbox" disabled="disabled" <c:if test="${fieldContents[26].fieldText == '4'}">checked</c:if> />品质部&nbsp;
        <input type="checkbox" disabled="disabled" <c:if test="${fieldContents[27].fieldText == '5'}">checked</c:if> />工程部 &nbsp;
    </c:otherwise>
</c:choose>
    </td>
		...
		<c:choose>
		    <c:when test="${E30.isEdit == '1'}">
		        <input type="Text" size="20" id="fieldContents[<c:out value="${E30.count}"/>].fieldText"
		            name="fieldContents[<c:out value="${E30.count}"/>].fieldText" value="<c:out value="${E25.fieldText}"/>" >
		    </c:when>
		    <c:otherwise>
