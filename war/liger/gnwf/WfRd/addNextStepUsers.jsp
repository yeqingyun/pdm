...
$(function(){
	//window.parent.parent.$("#wfRdwfNo").val();
	//alert("B4214700027");
	//alert("1111111111111111111111"+window.parent.parent.document.getElementById("wfRd.wfNo").value);
...
/* 	for(i=0; i<document.getElementById(id).options.length; i++) {
		if(i == 0){ */
			...
			usrString = document.getElementById(id).options[sel.selectedIndex].text;
		    usrIds = document.getElementById(id).options[sel.selectedIndex].value;

	
	var usrIdss = "userId"+userNameIDIt;
	var usrStrings = "userName"+userNameIDIt;
	
	/*  $("#"+usrIdss, window.parent.document).val(usrIds);
	$("#"+usrStrings, window.parent.document).val(usrString);   */
	//协办人
	//window.parent.selectAssistpProjectUserForFlow(usrIds,usrString,userNameIDIt);
	//主办人
	window.parent.selectprojectUserForFlow(usrIds,usrString,userNameIDIt);
	//parent.$(parent.document).data("userNameID",usrString);
	 window.parent.closepop();
}
...
<div id="eform" style="margin-left:15px;margin-right:15px;margin-top:15px;">
   ...
   <input type="hidden" id="userNameIDIt" name ="userNameIDIt" value="<c:out value="${userNameIDIt}"/>"/>
...
<form>
	<table width="100%"  >
	 ...
	 <table width="100%">
	  <tr>
	  <td></td>
	  <td align="right"><input type="button" class="wfbigbtn2" value="  确定   " onclick="saveMailUser('usrId')"/></td>
	  </tr>
	 </table>
</form>
</div></body>
