...
<head>
    ...
    <style type="text/css" rel="stylesheet">
        ...
        .borBRed {
            border-bottom: #ffd4c1 1px solid;
        }
        ...
        .titBox {
            background: url(./include/img/workflow/rpx.png) repeat-x 0 0;
            padding: 0 10px;
            height: 32px;
            line-height: 32px;
            border-bottom: #b7cee2 solid 1px;
        }
    ...
    <script type="text/javascript">
        ...
        function preUploadFile(prjtNo, wfNo, taskId, cateId) {
            ...
            var url = gngile_uploadURL
                ...
                + '&callBackUrl=' + server_URL + "/afterUploadAction!wfAfterUpload.dhtml"//回调的URL,对应你Action中的一个方法，在这个方法里面处理上传完后的业务逻辑
                + tempPar;
            ...
            var dialog = $.ligerDialog.open({
                url: url, height: 200, width: 490, buttons: [{
                    text: '完成',
        ...
        function deleteFile2(docId, fileNo) {			//删除附件
                        ...
                        $.post("WfRdView!deleteFile.shtml?wfDoc.docId=" + docId,
                            function (data) {
                                if (data.toString().indexOf("成功") > 0) {
                                    ...
                                    deleteFile('<%=OssConstant.server.getValue()%>','<%=OssConstant.code.getValue()%>', fileNo)
...
//							"text"
         ...
         **/
        var showUpload = function (prjtNo, wfNo, taskId, cateId) {
            var server_URL = $('#server_URL').val();
            var tempPar = 'tempParams=' + "wfNo:" + wfNo + ",taskId:" + taskId + ",cateId:" + cateId + ",uploadType:" + $("#uploadType").val();
            if (prjtNo) {
                tempPar = tempPar + ",prjtNo:" + prjtNo
            ...
            var data = {
                url: server_URL + "/afterUploadAction!wfAfterUpload.dhtml",
                param: tempPar
            };
            openUpload(${usrId}, data, "./WfRdView!viewDocCate.shtml?wfRd.wfNo=" + wfNo + "&currentTaskId=" + $("#currentTaskId").val() + "&taskStepId=" + $("#taskStepId").val(), window)
        }
...
<form id="wfRd-form" name="wfRddoc-form">
    ...
    <div align="center" style="margin-top: 10px">
        <c:if test="${ not empty currentStep.docName }">
            <font style="font-size: 14px;text-align: center;">本步骤需要上传<font style="font-size: 14px;color: red;"><c:out
                    value="${currentStep.docName}"/></font></font>
        </c:if>
    </div>
            ...
            <tr>
                <td>
                    <c:if test="${wfDoc.cateName!=null}">
                        ...
                        <input type="hidden" id="cate<c:out value="${wfDoc.cateId}"/>"
                               value="<c:out value="${wfDoc.cateName}"/>"/>
                    </c:if>
                    <c:if test="${wfDoc.docName==null&&wfDoc.isCurrntDoc==1}">
                        <input type="hidden" id="griddoccate<c:out value="${wfDoc.cateId}"/>" name="docCate"
                               value="<c:out value="${wfDoc.cateId}"/>"/>
                    </c:if>
                </td>
                <td>
                    <c:if test="${wfDoc.docName!=null}">
                        ...
                        <a href="WfDoc!dowPrjtDoc.shtml?wfDoc.docId=<c:out value="${wfDoc.docId}"/>">
                        <a href="WfDoc!dow.shtml?wfDoc.docId=<c:out value="${wfDoc.docId}"/>"> -->
                ...
                <td>
                    ...
                    <c:if test="${wfDoc.isCurrntDoc==1}">
                        <a href="javascript:preUploadFile('<c:out value="${wfRd.projectNo}"/>','<c:out value="${wfRd.wfNo}"/>',<c:out value="${currentTaskId}"/>,<c:out value="${wfDoc.cateId}"/>);"
                           class="colorBlue padLR5">添加</a>
                    </c:if>
                        <%-- <c:if test="${wfDoc.docId != null && wfDoc.isCurrntDoc==1}"> --%>
                    <c:if test="${wfRd.status==1 ||wfRd.status==0}">

                        <c:if test="${wfDoc.createBy == sessionScope.SYUSR.id }">
                            &nbsp;
                            <a href="javascript:deleteFile2(<c:out value="${wfDoc.docId}"/>,<c:out value="${wfDoc.fileNo}"/>);" class="colorBlue padLR5">删除</a>
                        </c:if>
                    </c:if>
                        <%-- </c:if>	 --%>
    ...
    <c:if test="${empty wfRd.docCateId }">
        <div class="wfbigbtn1">
                <%--<input id="uploadBut" type="button" value="  上传额外附件  " class="wfbigbtn2" onclick="javascript:preUploadFile('<c:out value="${wfRd.projectNo}"/>','<c:out value="${wfRd.wfNo}"/>',<c:out value="${currentTaskId}"/>,' ');">--%>
            <input id="uploadBut" type="button" value="  上传额外附件  " class="wfbigbtn2" onclick="showUpload('<c:out value="${wfRd.projectNo}"/>','<c:out value="${wfRd.wfNo}"/>',<c:out value="${currentTaskId}"/>,' ')">
        </div>
    </c:if>
