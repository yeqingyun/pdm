...
<input type="hidden" id="id" name="usr.id" size="30" value="<c:out value="${usr.id}"/>"/>
	<table width="90%">
		<tr>
			...
			<td>
				<select id="deptId" name="usr.deptId" style="width:135px">
					<c:forEach items="${depts}" var="dept">
						<option value="<c:out value="${dept.deptId}"/>"<c:if test="${dept.deptId==usr.orgNo}">selected</c:if>><c:out value="${dept.deptNm}"/></option>
					</c:forEach>
				</select>
			</td><td align="left"></td>
		</tr><tr>
			...
			<td align="left"></td>
			<td height="24" width="90" align="center">电子邮箱：</td><td><input type="text" id="email" name="usr.email" size="30" value="<c:out value="${usr.email}"/>" ltype="text" /></td>
			<td align="left"></td>
		</tr><tr>
