...
public class ComAction extends BasicAction {
	...
	private java.util.List<zrsy.vo.json.TreeJson> comJsons;
	...
	public String loadComs() throws Exception {
		try {
			coms = new zrsy.facade.ComFacade().find("select "+zrsy.vo.Com.SELF_FIELDS+" from Com where status=2 ",zrsy.vo.Com.SELF_FIELDS);
			//System.out.println("****"+"select "+zrsy.vo.Com.SELF_FIELDS+" from Com where status=2 ");
			for(int i=0;i<coms.size();i++){
				TreeJson treeJson = new TreeJson();
				treeJson.setId(coms.get(i).getComId());
				treeJson.setText(coms.get(i).getComNm());
				this.addtoComJsons(treeJson);
			}
			this.setJson(JSON.toJSONString(comJsons));
	...
	public String loadDept() throws Exception {
		try {
			depts = new zrsy.facade.DeptFacade().find("select "+ Dept.FIELDS+ " from Dept", Dept.FIELDS);
	...
	public String loadTree() throws Exception {
		try {
			...
			depts = new zrsy.facade.DeptFacade().find("select "+zrsy.vo.Dept.SELF_FIELDS+" from Dept where status=1 and Dept.Parent = "+com.getComId(),zrsy.vo.Dept.SELF_FIELDS);
			List<Dept> cdepts = new zrsy.facade.DeptFacade().find("select "+zrsy.vo.Dept.SELF_FIELDS+" from Dept where status=1 and Dept.Parent in(select dept.deptid from dept where Dept.parent="+com.getComId()+")",zrsy.vo.Dept.SELF_FIELDS);
			for(int i=0; i<depts.size(); i++) {
				TreeJson treeJson = new TreeJson();
				treeJson.setId(depts.get(i).getDeptId());
				treeJson.setText(depts.get(i).getDeptNm());
				...
				for(int j=0; j<cdepts.size(); j++) {
					if(depts.get(i).getDeptId()==cdepts.get(j).getParent()){
						TreeJson children = new TreeJson();
						children.setId(cdepts.get(j).getDeptId());
						children.setText("&nbsp;&nbsp;"+cdepts.get(j).getDeptNm());
						this.addtoTreeJsons(children);
						List<Dept> cdepts1 = new zrsy.facade.DeptFacade().find("select "+zrsy.vo.Dept.SELF_FIELDS+" from Dept where status=1 and Dept.Parent inselect dept.deptid from dept where Dept.parent="+cdepts.get(j).getDeptId()+")",zrsy.vo.Dept.SELF_FIELDS);
						if(cdepts1.size()>0){
							for(int k=0;k<cdepts1.size();k++){
								TreeJson cchildren = new TreeJson();
									cchildren.setId(cdepts1.get(k).getDeptId());
									cchildren.setText("&nbsp;&nbsp;&nbsp;&nbsp;"+cdepts1.get(k).getDeptNm());
									this.addtoTreeJsons(cchildren);
							}
						}
	...
	public String sav() throws Exception {
		try {
			...
			if(com.getComId() == null){
				com.setCreateBy(getUsrSession().getId());
				com.setCreateDate(new Date());
				new ComFacade().save(com);
			}else{
				com.setLastUpd(getUsrSession().getId());
				com.setLastDate(new Date());
	...
	public java.util.List<zrsy.vo.json.TreeJson> getComJsons() {
		return comJsons;
	}
	public void setComJsons(java.util.List<zrsy.vo.json.TreeJson> comJsons) {
		this.comJsons = comJsons;
	}
	...
	public void addtoComJsons(zrsy.vo.json.TreeJson treeJson) {
		if(getComJsons() == null) setComJsons(new ArrayList<zrsy.vo.json.TreeJson>());
		getComJsons().add(treeJson);
	}
}