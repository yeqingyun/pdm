...
public class ToolBar0Action extends BasicAction {
	...
	public String execute() throws Exception {
		try {
			...
			List<Btn> btns = new BtnFacade().find("select distinct "+Btn.SELF_FIELDS+" from Btn" +
					...
					" inner join PgBtn on(PgBtn.NodeId = Node.Id and PgBtn.BtnId = Btn.Id and PgBtn.PgTyp = 0)" +
			...
			for(int i=0; i<btns.size(); i++) {
				ligerToolBar.addtoItems(new LiToolBarData(btns.get(i).getId().toString(),btns.get(i).getText(),btns.get(i).getClick().toString(), null, btns.get(i).getImg(),null,null));
			}
			
			setJson(JSON.toJSONString(ligerToolBar));
			System.out.println(JSON.toJSONString(ligerToolBar));
		}
		catch(Exception e) { 
			setMsg(MSG.F_SEA);
