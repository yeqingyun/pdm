...
public class ToolBarAction extends BasicAction {
	...
	public String execute() throws Exception {
		try {
			...
			List<Btn> btns = new BtnFacade().find("select distinct "+Btn.SELF_FIELDS+" from Btn" +
					...
					" where Node.Url = '"+parm+".shtml'",Btn.SELF_FIELDS);
			
			for(int i=0; i<btns.size(); i++) {
				ligerToolBar.addtoItems(new LiToolBarData(btns.get(i).getId().toString(),btns.get(i).getText(),btns.get(i).getClick(), null, null, null));
			}
	...
	public String custom() throws Exception {
		try {
			...
			String sql = "select distinct "+Btn.SELF_FIELDS+" from Btn" +
							" inner join GpBtn on(Btn.Id = GpBtn.BtnId) " +
							" inner join Node on(GpBtn.NodeId = Node.Id) " +
							" inner join PgBtn on(PgBtn.NodeId = Node.Id and PgBtn.BtnId = Btn.Id and PgBtn.PgTyp = "+pageType+")" +
							" where Node.Url = '"+parm+".shtml' and Btn.SyId = "+this.getUsrSession().getSyId()+
							" order by Btn.sort";
			List<Btn> btns = new BtnFacade().find(sql,Btn.SELF_FIELDS);
