...
public class DeptHelper extends BasicDeptHelper {
	public String getSqlString() {
		return " from Dept " +
               " inner join Com on (Com.ComId = Dept.parent) " + 
			...
			while(rs.next()) {
				...
				for(int i=0;i<_fields.length;i++) {
					...
					if(_fields[i].equals("ComNm") || _fields[i].equals("Com.ComNm as ComNm"))
						dept.setComNm(rs.getString("ComNm"));
					
					else if(_fields[i].equals("ischecked") || _fields[i].equals("Dept.ischecked")){
						if(rs.getString("ischecked")!=null){
							dept.setIschecked("checked");
						}
					}
