		...
		try {
			while(rs.next()) {
				...
				for(int i=0;i<_fields.length;i++) {
					...
					else if(_fields[i].equals("IsWide") || _fields[i].equals("Usr.IsWide"))
						usr.setIsWide(rs.getInt("IsWide"));
		...
		try {
			...
			for(int i=0;i<_fields.length;i++) {
					if(_fields[i].equals("Id") || _fields[i].equals("Usr.Id")) {
						...
						index++;
						DbConnUtil.getDbconn().getCurrentPstmt().setInt(index, usr.getIsWide());
					}
	...
	public String getUpdateSql(String fields,String key,String keyValue) {
		...
		for(int i=0;i<_fields.length;i++) {
					...
					if(_fields[i].equals("LastDate") || _fields[i].equals("Usr.LastDate"))
						sql += _fields[i] + " = ?,";
					if(_fields[i].equals("IsWide") || _fields[i].equals("Usr.IsWide"))
						sql += _fields[i] + " = ?,";
